...
\section{b)}
  \G{} ne peut pas être cyclique dû aux contraintes stipulées. Une contrainte particulière, ci-dessous, en témoigne.
  \begin{siderules}
    Il n'y a \textit{pas} de corridor passant d'un bassin vers lui-même.
  \end{siderules}

  Il est impossible qu'un cycle soit établi avec un seul bassin considérant qu'il y a aucun corridor qui part d'un dit bassin en arrivant dans ce même bassin.\bigskip
...
\section{f)}
  ...
  \begin{algorithm}[H]
    ...
    \KwIn{Liste d'adjacence, tableau asssociatif $S: [n] \times [n] \rightarrow \mathbb{N} > 0$.}
...
\begin{algorithm}[H]
  ...
  \Def{\TMode($x$)}{
    ...
    \If{$|x|=0$}{
      \Return{$mode$}\tcp*{1\textsuperscript{re} condition d'arret.}
      }
    \Else{
      ...
      \Else{
        \TMode{$gauche$}\;
        \TMode{$droite$}\tcp*{2\textsuperscript{e} cond. d'arret $\neq$ trouverMode(\textit{centre}).}
      }
    }
  }
  \Return{\TMode{$S$}}  
...
\begin{algorithm}[H]
  ...
  \KwIn{Un graphe\G{} tel que \G{}$=(\V{},\E{})$ est non dirié et un sommet $u \in \V{}$}
  ...
  \Def{\Vis{$x$}}{
    ...
    \ForAll{voisins de $x \in \V{} : x \rightarrow y$}{
      \If{$y$ non marqué}{
        \Vis{$y$} \tcp*{Récursif, mais jamais plus d'une fois par noeud.}
      }      
    }
  }
  \Vis{$u$}\;
...
Cet algorithme parcours le graphe au complet. Si un sommet est visité plus d'une fois, c'est cyclique.
Ceci dit, cet algorithme se rappele sur chacun des noeuds un maximum d'une fois.
À la fin de son exécution, si la variable $Visites = |\V{}|$, nous avons bien un arbre! 
Ainsi, nous pouvons affirmer que le temps d'exécution maximal correspondra à $\mathbfcal{O}(|V|)$. Définissant $n$ pour le nombre de sommet, le temps est alors de $\mathbfcal{O}(n)$.
\newpage
\section{b)}
...
Dans cette image, on retrouve un arbre couvrant minimal qui couvre $4$ sommets d'un graphe\G{} à $4$ sommets.
Assumant qu'il y a $1$ unité ($u$) de distance entre les coins, du carré formé, adjacants, le grapheaurait une longueur suivant l'équation ci-dessus.
...
\begin{tikzpicture}[node distance=2cm]
  ...
  \node[state] [below of=0] (2) {};
  \node[state] [below of=1] (3) {};
  \node[text width=4cm, anchor=west, right, align=center] at (4, -1){$4\cdot(\frac{1}{2})\cdot\sqrt{(1^2 + 1^2)}=2\cdot \sqrt{2} =2.8284\dots u$};
